...
-(void)creatUI{
    ...
    UIImageView *weatherImg = [[UIImageView alloc]initWithFrame:CGRectMake(kScreenWidth/2-50*widthPix, CGRectGetMaxY(_cityBt.frame) + 30, 100 * widthPix, 100 * heightPix)];
    weatherImg.tag = 11;
    [self addSubview:weatherImg];
    
    
    UILabel *tempLable = [[UILabel alloc]initWithFrame:CGRectMake(50, CGRectGetMaxY(weatherImg.frame), (kScreenWidth - 100), 40 * heightPix)];
    tempLable.font = [UIFont boldSystemFontOfSize:30 *heightPix];
    ...
    for (NSInteger i = 0; i < 2; i++) {
        ...
        UILabel *dayLabel = [[UILabel alloc]initWithFrame:CGRectMake((kScreenWidth/2-100*widthPix)/2 + kScreenWidth/2*i, CGRectGetMaxY(tempLable.frame)+50, 100 * widthPix, 20 *heightPix)];
